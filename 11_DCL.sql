...
* 사용자 계정 : 데이터베이스에 대하여 질의, 갱신, 보고서 작성 등의
              ...
              ex) kdh계정(각자 이니셜 계정), workbook 등
*/

/* DCL(Data Control Langauge) : 계정에 DB, DB객체에 대한 접근 권한을 부여하고 회수하는 언어
...
CREATE USER kdh_sample IDENTIFIED BY sample1234;
        /* 사용자 계정 명 */          /* 비밀번호 */
-- User KDH_SAMPLE이(가) 생성되었습니다.

-- 2. 생성된 계정에 접속하기 위한 접속 방법을 추가(왼쪽 상단 초록 + 버튼)
--> ORA-01045 : CREATE SESSION 권한을 가지고 있지 않아 로그인 실패

-- 3. [SYS 관리자 계정] 생성된 샘플 계정에 DB 접속 권한 부여
GRANT CREATE SESSION TO kdh_sample; -- Grant을(를) 성공했습니다.
...
--> DB 접속 권한만 있는 상태, 테이블 생성 권한 없음

-- 6. [SYS 관리자 계정] 테이블 생성 권한 부여 + 객체 생성 공간 할당
GRANT CREATE TABLE TO kdh_sample; -- Grant을(를) 성공했습니다.
ALTER USER kdh_sample DEFAULT TABLESPACE SYSTEM QUOTA UNLIMITED ON SYSTEM;
...
--> kdh_sample 계정에서 kdh 계정의 EMPLOYEE 테이블 조회

-- 1. [샘플 계정] kdh계정의 EMPLOYEE 테이블 조회
SELECT * FROM kdh.EMPLOYEE; --> EMPLOYEE 테이블(객체) 조회 권한 없음

-- 2. [이니셜 계정] 샘플 계정에 EMPLOYEE 테이블 조회 권한 부여
GRANT SELECT ON EMPLOYEE TO kdh_sample; -- Grant을(를) 성공했습니다.

-- 3. [샘플 계정] 다시 EMPLOYEE 테이블 조회
SELECT * FROM kdh.EMPLOYEE;

-- 4. [이니셜 계정] 부여했던 SELECT 권한 회수
REVOKE SELECT ON EMPLOYEE FROM kdh_sample; 

-- 5. [샘플 계정] 다시 EMPLOYEE 테이블 조회
SELECT * FROM kdh.EMPLOYEE; --> 조회 안됨
