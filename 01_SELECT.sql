...
--            디벨로퍼의 날짜 표기 방법이 년/월/일 로 지정되어 있는 것이다.)
...
-- SELECT 조회 결과인 집합인 RESULT SET에 컬럼명을 지정
...
/* BETWEEN을 이용한 날짜 비교 */
-- EMPLOYEE 테이블에서
-- 입사일이 1990/01/01 ~ 1999/12/31일 (90년도 입사자)
-- 직원의 사번, 이름, 입사일 조회
...
-- EMPLOYEE 테이블에서 보너스를 받는 사람의 이름, 보너스 조회
...
FROM EMPLOYEE
ORDER BY SALARY ASC; --(*ASENDING 값이 점점 커짐)
