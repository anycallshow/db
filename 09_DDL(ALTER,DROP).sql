-- DDL(Date Definition Language) : 데이터 정의 언어로
...
ALTER TABLE DEPT_COPY DROP CONSTRAINT SYS_C007482; -- DEPT_COPY 테이블 들어가서 제약조건에 있는 이름 확인하고 복사함 


-- NOT NULL 제거 시 MODIFY 사용
ALTER TABLE DEPT_COPY 
MODIFY DEPT_TITLE CONSTRAINT SYS_C007483 NULL;
--     (컬럼 이름)
...
--> 제약조건까지 모두 삭제
-- == FK 제약조건으로 인해 삭제가 원래는 불가능 하지만, 제약조건을 없어버려서 FK 관계를 해제
...
-- 1) 컬럼명 변경 : ALTER TABLE 테이블명 RENAME COLNUMN 컬럼명 TO 컬럼명
