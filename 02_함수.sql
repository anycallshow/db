...
-- 그룹(GROUP) 함수 : N개의 값의 읽어 1개의 결과 반환 
...
SELECT '----K  H----', TRIM(BOTH '-' FROM '----K  H----') FROM DUAL; -- 양쪽 공백 제거 (중간 미포함)
...
SELECT 123.456, ROUND(123.465, 1) FROM DUAL; -- 소수점 첫째 자리까지 출력 == 소수점 둘째 자리에서 반올림

SELECT 123.456, ROUND(123.465, 2) FROM DUAL;

SELECT 123.456, ROUND(123.465, 0) FROM DUAL; -- 소수점 첫째 자리에서 반올림
SELECT 123.456, ROUND(123.465, -1) FROM DUAL; -- 소수점 0번째 자리에서 반올림
SELECT 123.456, ROUND(123.465, -2) FROM DUAL; -- 소수점 -1번째 자리에서 반올림
...
SELECT ROUND(MONTHS_BETWEEN(SYSDATE, '2023/10/24')) || '개월' 수강기간
FROM DUAL;
...
--> 기존에 없던 패턴을 추가 시 ""(쌍따옴표)로 감싸줘서 문자열 그대로를 출력하게 함
...
SELECT '2023-11-21', TO_DATE('2023-11-21') FROM DUAL;
...
SELECT TO_CHAR(TO_DATE('20231121 173000', 'YYMMDD HH24MISS'), 'YYYY/MM/DD HH24"시"MI"분"') FROM DUAL;
...
-- 이름, 기본 보너스, 변경된 보너스 조회
SELECT EMP_NAME 이름, BONUS 보너스, NVL2(BONUS, 0.8, 0.3) "변경된 보너스"
FROM EMPLOYEE;
...
SELECT EMP_NAME, JOB_CODE,
    -- 급여 * (1 + 보너스) * 12
    TO_CHAR(SALARY * (1 + NVL(BONUS, 0)) * 12, 'L999,999,999') "연봉(원)"
FROM EMPLOYEE;
...
-- COUNT([DISTINCT] 컬럼명 ) : 중복을 제거한 행 개수 헤아려서 리턴
...
-- COUNT(컬럼명) : NULL을 제외한 실제 값이 기록된 행 개수 리턴함 
SELECT COUNT(DEPT_CODE) FROM EMPLOYEE;

-- EMPLOYEE 테이블에 있는 부서 개수
SELECT COUNT(DISTINCT DEPT_CODE) FROM EMPLOYEE;
--> 7행의 결과가 있지만 1행이 NULL이기 때문에 COUNT시 제외되어 6행으로 조회됨
