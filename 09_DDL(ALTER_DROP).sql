...
-- ALTER(바꾸다, 변조하다)
-- 수정 가능한 것 : 컬럼(추가/수정/식제), 제약조건(추가/삭제).
--               이름변경(테이블, 컬럼, 제약조건)
...
ALTER TABLE DEPT_COPY DROP CONSTRAINT SYS_C007500;


-- NOT NULL 제거 시 MODIFY 사용
ALTER TABLE DEPT_COPY 
MODIFY DEPT_TITLE CONSTRAINT SYS_C007501 NULL;
----   (컬럼 이름)
...
SELECT * FROM DEPT_COPY;
--> ROLLBACK 안됨!
-- WHY? CREATE / ALTER / DROP 같은 DDL은 ROLLBACK의 대상이 아님
...
--> 결론 : DML /DDL 혼용하여 사용하지 말자

INSERT INTO DEPT_COPY VALUES('D0'); -- 'D0' 삽입
SELECT * FROM DEPT_COPY;
ROLLBACK; -- 트랜잭션에서 'D0' INSERT 내용 삭제
...
-- [작성법]
-- DROP TABLE 테이블명 [CASCADE CONSTRIANTS];
