...
-- 2) 직원들 중 급여가 3047633원 이상인 사원들의 사번, 이름, 직급코드, 급여 조회
...
/* 서브쿼리 유형
    ...
    - 상관 서브쿼리 : 서브쿼리가 만든 결과 값을 메인 쿼리가 비교 연살할 때
...
FROM EMPLOYEE
GROUP BY DEPT_CODE; -- 17700000 (D9)
...
-- 3) 최종적으로 EMPLOYEE 테이블에서 위의 결과들과 동일한 DEPT_CODE를 가지는 직원 조회
...
-- 1) 급여를 200, 600만 받는 직원 (200만, 600만 평균급여라 생각할 경우)
SELECT EMP_ID, EMP_NAME, JOB_CODE, SALARY
FROM EMPLOYEE
...
FROM EMPLOYEE
ORDER BY SALARY DESC;
--> 해결 1의 조회 결과(RESULT SET)을 가상의 테이블(== VIEW) 취급할 예정
...
FROM EMPLOYEE E1
LEFT JOIN JOB J ON(E1.JOB_CODE = J.JOB_CODE)
...
SELECT FLOOR(MONTHS_BETWEEN(SYSDATE, TO_DATE(SUBSTR(EMP_NO, 1, 6), 'RRMMDD')) / 12) 만나이
FROM EMPLOYEE;
