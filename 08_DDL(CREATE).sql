...
CREATE TABLE MEMBER(
    ...
    MEMBER_SSN CHAR(14), -- 990808-1234567 주민번호 13글자 + '-' 고정된 14글자
...
-- 만들 테이블 확인
SELECT * FROM MEMBER;
SELECT * FROM USER_TABLES; -- 딕셔너리뷰(데이터 딕셔너리에서 일부분만 뽑아서 만든 가상 테이블)
...
-- 추가 샘플 데이터 삽입
-- 가입일 -> SYSDATE를 활용
INSERT INTO MEMBER VALUES('MEM02', 'QWER1234', '김영희', '980112-1111222', SYSDATE);
COMMIT;
SELECT * FROM MEMBER;

-- 가입일 -> DEFAULT 활용(테이블 생성 시 정의된 값이 반영됨)
INSERT INTO MEMBER VALUES('MEM03', 'ASDFQWER', '박철수', '951212-2345678', DEFAULT);
COMMIT;
SELECT * FROM MEMBER;

-- 가입일 -> INSERT 시 미작성 하는 경우 -> DEFAULT 값이 반영됨
INSERT INTO MEMBER(MEMBER_ID, MEMBER_PWD, MEMBER_NAME, MEMBER_SSN) 
VALUES('MEM04', '12341234', '이나라', '000101-4564567');
COMMIT;
SELECT * FROM MEMBER;
...
INSERT INTO MEMBER2 VALUES('김영희', 0011271234567);
SELECT * FROM MEMBER2;
...
INSERT INTO USER_USED_NN
VALUES(NULL, NULL, NULL, NULL, NULL, '010-1234-1234', 'hong123@kh.or.kr');
--> NOT NULL 제약 조건에 위배되어 오류 발생
-- ORA-01400: NULL을 ("KDH"."USER_USED_NN"."USER_NO") 안에 삽입할 수 없습니다
...
CREATE TABLE USER_USED_UK(
    USER_NO NUMBER,
    -- USER_ID VARCHAR2(20) UNIQUE, -- 컬럼레벨 제약조선 설정 (제약조건명 미지정)
...
-- ORA-00001: 무결성 제약 조건(KDH.USER_ID_U)에 위배됩니다
                        --> UNIQUE
...
INSERT INTO USER_USED_UK2
VALUES(2, 'user02', 'pass01', '고길동', '남', '010-1234-1234', 'hong123@kh.or.kr');
--> USER_NAME이 다름 -> 삽입 성공
...
-- ORA-00001: 무결성 제약 조건(KDH.USER_ID_NAME_U)에 위배됩니다
...
-- NOU NULL + UNIQUE 제약조건의 의미

-- 한 테이블당 한 개만 설정할 수 있음
-- 컬럼레벨, 테이블레벨 둘 다 설정 가능함
-- 한 개 컬럼에 설장할 수도 있고, 여러개의 컬럼을 묶어서 설정할 수 있음(복합키)
...
INSERT INTO USER_USED_PK
VALUES(1, 'user02', 'pass02', '김영희', '여', '010-4567-1234', 'kim123@kh.or.kr');
--> 기본키 중복으로 오류
-- ORA-00001: 무결성 제약 조건(KDH.USER_NO_PK)에 위배됩니다 == UNIQUE 오류와 같음


INSERT INTO USER_USED_PK
VALUES(NULL, 'user03', 'pass03', '박철수', '남', '010-4567-1244', 'park123@kh.or.kr');
--> 기본키가 NULL이므로 오류
-- ORA-01400: NULL을 ("KDH"."USER_USED_PK"."USER_NO") 안에 삽입할 수 없습니다 == NOT NULL 오류와 같음
...
INSERT INTO USER_USED_PK2
VALUES(2, 'user01', 'pass01', '박철수', '남', '010-4567-1244', 'park123@kh.or.kr');

INSERT INTO USER_USED_PK2
VALUES(1, 'user01', 'pass01', '김길동', '남', '010-1234-1234', 'hong123@kh.or.kr');
-- 회원 번호와 아이디 둘 다 중복 되었을 때만 제약조건 위배 에러 발생 
-- ORA-00001: 무결성 제약 조건(KDH.PK_USERNO_USERID)에 위배됩니다

-- PRIMARY KEY는 NULL이 들어갈 수 없음
SELECT * FROM USER_USED_PK2;
INSERT INTO USER_USED_PK2
VALUES(NULL, 'user01', 'pass01', '김길동', '남', '010-1234-1234', 'hong123@kh.or.kr');
...
-- 컬럼레벨일 경우
-- 컬럼명 자료형(크기) [CONSTRAINT 이름] REFERNCES 참조할 테이블명 [(참조할컬럼)] [삭제룰]
...
-- 참조할 테이블이 참조할 컬럼명이 생략되면, PRIMARY KEY로 설정된 컬럼이 자동 참조할 컬럼이 됨
...
CREATE TABLE USER_USED_FK(
    ...
    PHONE VARCHAR2(30),
    EMAIL VARCHAR2(50),
    GRADE_CODE NUMBER -- CONSTRAINT GRADE_CODE_FK REFERENCES USER_GRADE(GRADE_CODE) -- 컬럼 레벨에 FK 설정
    ,
...
VALUES(1, 'user01', 'pass01', '홍길동', '남', '010-1234-1234', 'hong123@kh.or.kr', 10);

INSERT INTO USER_USED_FK
VALUES(2, 'user02', 'pass02', '김영희', '여', '010-4567-1234', 'kim123@kh.or.kr', 20);

INSERT INTO USER_USED_FK
VALUES(3, 'user03', 'pass03', '박철수', '남', '010-4567-1244', 'park123@kh.or.kr', 30);

INSERT INTO USER_USED_FK
VALUES(4, 'user04', 'pass04', '김길동', '남', '010-1234-1234', 'hong123@kh.or.kr', NULL);
--> NULL 사용 가능

SELECT * FROM USER_USED_FK;

INSERT INTO USER_USED_FK
VALUES(5, 'user05', 'pass05', '고길동', '남', '010-1234-1234', 'go123@kh.or.kr', 50);
--> 50이라는 값은 USER_GRADE 테이블 GRADE_CODE 컬럼에서 제공하는 값이 아니므로
-- 외래키 제약 조건에 위배되어 오류 발생
-- ORA-02291: 무결성 제약조건(KDH.GRADE_CODE_FK)이 위배되었습니다- 부모 키가 없습니다
            --> 참조 무결성 위배
...
WHERE GRADE_CODE = 10;
-- ORA-02292: 무결성 제약조건(KDH.GRADE_CODE_FK)이 위배되었습니다- 자식 레코드가 발견되었습니다
...
INSERT INTO USER_USED_FK2
VALUES(2, 'user02', 'pass02', '김영희', '여', '010-4567-1234', 'kim123@kh.or.kr', 10);

INSERT INTO USER_USED_FK2
VALUES(3, 'user03', 'pass03', '박철수', '남', '010-4567-1244', 'park123@kh.or.kr', 30);

INSERT INTO USER_USED_FK2
VALUES(4, 'user04', 'pass04', '김길동', '남', '010-1233-1234', 'dong123@kh.or.kr', NULL);
...
--> ON DELETE SET NULL 옵션이 설정되어 있어서 오류없이 삭제됨.
...
SELECT * FROM USER_GRADE3; -- 부모 행 삭제 확인(10 없음)
...
INSERT INTO USER_USED_CHECK
VALUES(2, 'user01', 'pass01', '홍길동', '남자', '010-1234-1234', 'hong123@kh.or.kr');
-- GENDER 컬럼에 CHECK 제약조건으로 '남' 또는 '여'만 기록 가능한데 '남자'라는 조건 이외의 값이 들어와 에러 발생
-- ORA-02290: 체크 제약조건(KDH.GENDER_CHECK)이 위배되었습니다
...
CREATE TABLE USER_TEST(
    ...
    STATUS VARCHAR2(10) CONSTRAINT NN_STATUS_CK_STATUS CHECK( STATUS IN('Y', 'N') ) NOT NULL,
    
    CONSTRAINT PK_USERTEST_PNO PRIMARY KEY(USER_NO, PNO)
);
...
CREATE TABLE USER_TEST(
    ...
    PNO VARCHAR2(20) CONSTRAINT UK_PNO UNIQUE CONSTRAINT NN_PNO NOT NULL, -- 컬럼레벨에서 여러개 작성가능
...
INSERT INTO USER_TEST
VALUES(2, 'user02', 'pass02', '890122-1234567', '남', '010-2222-9999', '서울시 강남구 삼성동', 'N');

INSERT INTO USER_TEST
VALUES(3, 'user03', 'pass03', '910122-1234567', '남', '010-3333-9999', '서울시 강남구 청담동', 'N');

INSERT INTO USER_TEST
VALUES(4, 'user04', 'pass04', '920122-2234567', '여', '010-4444-9999', '서울시 강남구 도곡동', 'N');

INSERT INTO USER_TEST
VALUES(5, 'user05', 'pass05', '880122-2234567', '여', '010-5555-9999', '서울시 강남구 대치동', 'N');
