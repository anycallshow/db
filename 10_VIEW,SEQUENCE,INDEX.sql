...
GRANT CREATE VIEW TO kdh; -- Grant을(를) 성공했습니다.
...
-- ORA-01400: NULL을 ("KDH"."DEPT_COPY2"."DEPT_TITLE") 안에 삽입할 수 없습니다
...
WITH READ ONLY; -- 읽기 전용 VIEW 생성 (DML X)
...
-- 시퀀스의 캐시 메모리는 할당된 크기만큼 미리 다음 값들을 생성해서 저장해둠
    --> 시퀀스 호출 시 미리 저장된 값들을 가져와 반환하므로
        매번 시퀀스를 생성해서 반환하는 것 보다 DB속도가 향상됨

        ** 시퀀스 사용 방법 **
        
        1) 시퀀스명.NEXTVAL : 다음 시퀀스 번호를 얻어옴. (INCREMENT BY 만큼 증간된 값)
                            단, 시퀀스 생성 후 첫 호출인 경우 START WITH의 값을 얻어옴.
                            
        2) 시퀀스명.CURRVAL : 현재 시퀀스 번호를 얻어옴.
                            단, 시퀀스 생성 후 NEXTVAL 호출 없이 CURRVAL 호출하면 오류 발생
...
* 인덱스 단점
 - 데이터 변경(INSERT, UPDATE, DELETE) 작업이 빈번한 경우 오히려 성능이 저하되는 문제가 발생
...
-- Index ECOPY4_NAME_INDEX이(가) 생성되었습니다.
...
WHERE TEST_ID = 'TEST500000'; -- 0.02초
